...
func NewAPI(usersRepo repo.UserRepository, sessionsRepo repo.SessionsRepository, products repo.ProductRepository, cartsRepo repo.CartRepository) API {
	...
	mux.Handle("/page/register", api.Get(http.HandlerFunc(Page{File: "register.html"}.ServeHTTP)))
	mux.Handle("/page/login", api.Get(http.HandlerFunc(Page{File: "login.html"}.ServeHTTP)))
	mux.Handle("/page/dashboard", api.Get(api.Auth(http.HandlerFunc(Page{File: "dashboard.html"}.ServeHTTP))))
